...
- (void)loadView {
	...
	keywordTableView = [[UITableView alloc] initWithFrame:CGRectMake(0.0, 0.0, bounds.size.width, bounds.size.height - 44.0)];
...
-(void)viewWillAppear:(BOOL)animated{
	...
	// ハイライト解除
	[keywordTableView deselectRowAtIndexPath:[keywordTableView indexPathForSelectedRow] animated:YES];
}
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
	return [bloglistParser.keywordData count];
}
...
- (void)dealloc {
	[selectedKeyword release];
	[keywordTableView release];
	[toolBar release];
	[bloglistParser release];
