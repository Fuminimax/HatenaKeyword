...
- (void)loadView {
	...
	searchListTableView = [[UITableView alloc] initWithFrame:CGRectMake(0.0, 0.0, bounds.size.width, bounds.size.height - 44.0)];
...
-(void)viewWillAppear:(BOOL)animated{
	...
	// ハイライト解除
	[searchListTableView deselectRowAtIndexPath:[searchListTableView indexPathForSelectedRow] animated:YES];
}
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
	//NSInteger *ret = [searchListParser.keywordData count]; 
	//NSLog(@"searchListParser.keywordData.count: %d", ret);
...
- (void)dealloc {
	[searchWord release];
	[searchListTableView release];
	[searchListParser release];
